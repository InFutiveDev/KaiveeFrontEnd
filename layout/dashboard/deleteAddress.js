...
const DeleteAddress = ({ addressToDelete, setAddressToDelete }) => {
    const dispatch = useDispatch();
    const [loading, setLoading] = useState(false);

    const deleteAddress = async () => {
        setLoading(true);
        const res = await dispatch(DELETE_USER_ADDRESS(addressToDelete?._id));
        if (res?.success) {
            setAddressToDelete(null);
            dispatch(GET_USER_ADDRESSES()); // Refresh the list of addresses after deletion
        }
        setLoading(false);
    };
    ...
    return (
        <>
            <Modal
                ...
                toggle={() => setAddressToDelete(null)}
            >
